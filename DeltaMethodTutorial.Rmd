...
In epidemiology we use routinely the delta method to compute the standard error (SE) of the risk difference (RD), the risk ratio (RR), and the odds ratio (OR) [@Agresti2010]. Often in addition to reporting parameters fitted by a model, we need to report some conditional transformation of these parameters. The transformation can generate the point estimates of our desired values, but the SE of these point estimates are not so easily calculated. 
...
lb <- mrr - 1.96*sqrt(vG)
ub <- mrr + 1.96*sqrt(vG)
cat("\n Conditional Risk Ratio (95%CI): ") ; cat(mrr, "(", lb,",", ub,")")
```
