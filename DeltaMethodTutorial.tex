...
\NormalTok{gm <-}\StringTok{ }\KeywordTok{glm}\NormalTok{(Y }\OperatorTok{~}\StringTok{ }\NormalTok{age }\OperatorTok{+}\StringTok{ }\NormalTok{cmbd, }\DataTypeTok{data =}\NormalTok{ data, }\DataTypeTok{family =} \KeywordTok{binomial}\NormalTok{(}\DataTypeTok{link=}\NormalTok{logit))}
...
\NormalTok{Q1W <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(gm, }\DataTypeTok{newdata =} \KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{cmbd =} \DecValTok{1}\NormalTok{, }\DataTypeTok{age =}\NormalTok{ data[ ,}\KeywordTok{c}\NormalTok{(}\StringTok{"age"}\NormalTok{)]), }\DataTypeTok{type =} \StringTok{"response"}\NormalTok{)}
\NormalTok{Q0W <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(gm, }\DataTypeTok{newdata =} \KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{cmbd =} \DecValTok{0}\NormalTok{, }\DataTypeTok{age =}\NormalTok{ data[ ,}\KeywordTok{c}\NormalTok{(}\StringTok{"age"}\NormalTok{)]), }\DataTypeTok{type =} \StringTok{"response"}\NormalTok{)}
\CommentTok{# Step 2 estimation and prediction of the propensity score (ps): E(A|W) or E(cmbd|age)}
\NormalTok{psm <-}\StringTok{ }\KeywordTok{glm}\NormalTok{(cmbd }\OperatorTok{~}\StringTok{ }\NormalTok{age}\OperatorTok{*}\NormalTok{age}\OperatorTok{*}\NormalTok{age, }\DataTypeTok{family =}\NormalTok{ binomial, }\DataTypeTok{data =}\NormalTok{ data)}
\NormalTok{gW =}\StringTok{ }\KeywordTok{predict}\NormalTok{(psm, }\DataTypeTok{type =} \StringTok{"response"}\NormalTok{)}
\NormalTok{gbounds <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(x,}\DataTypeTok{bounds=}\KeywordTok{c}\NormalTok{(}\FloatTok{0.01}\NormalTok{,}\DecValTok{1}\NormalTok{))\{}
\NormalTok{  x[x}\OperatorTok{<}\KeywordTok{min}\NormalTok{(bounds)] <-}\StringTok{ }\KeywordTok{min}\NormalTok{(bounds)}
\NormalTok{  x[x}\OperatorTok{>}\KeywordTok{max}\NormalTok{(bounds)] <-}\StringTok{ }\KeywordTok{max}\NormalTok{(bounds)}
  \KeywordTok{return}\NormalTok{(x)}
\NormalTok{\}}
\CommentTok{#gW <- gbounds(gW,bounds=c(0.05,0.975))}
\CommentTok{# MCRR}
\NormalTok{EY1 <-}\StringTok{ }\KeywordTok{mean}\NormalTok{((data}\OperatorTok{$}\NormalTok{Y) }\OperatorTok{*}\StringTok{ }\NormalTok{(data}\OperatorTok{$}\NormalTok{Y }\OperatorTok{-}\StringTok{ }\NormalTok{Q1W) }\OperatorTok{/}\StringTok{ }\NormalTok{gW }\OperatorTok{+}\StringTok{ }\NormalTok{Q1W)}
\NormalTok{EY0 <-}\StringTok{ }\KeywordTok{mean}\NormalTok{((}\DecValTok{1} \OperatorTok{-}\StringTok{ }\NormalTok{data}\OperatorTok{$}\NormalTok{Y) }\OperatorTok{*}\StringTok{ }\NormalTok{(data}\OperatorTok{$}\NormalTok{Y }\OperatorTok{-}\StringTok{ }\NormalTok{Q0W) }\OperatorTok{/}\StringTok{ }\NormalTok{(}\DecValTok{1} \OperatorTok{-}\StringTok{ }\NormalTok{gW) }\OperatorTok{+}\StringTok{ }\NormalTok{Q0W)}
\NormalTok{logMCRR <-}\StringTok{ }\KeywordTok{log}\NormalTok{(EY1) }\OperatorTok{-}\StringTok{ }\KeywordTok{log}\NormalTok{(EY0) }
\NormalTok{MCRR <-}\StringTok{ }\KeywordTok{exp}\NormalTok{(logMCRR);MCRR}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Functional Delta Method for statisical inference}
\NormalTok{D1 <-}\StringTok{ }\NormalTok{(data}\OperatorTok{$}\NormalTok{cmbd) }\OperatorTok{*}\StringTok{ }\NormalTok{(data}\OperatorTok{$}\NormalTok{Y }\OperatorTok{-}\StringTok{ }\NormalTok{Q1W) }\OperatorTok{/}\StringTok{ }\NormalTok{gW }\OperatorTok{+}\StringTok{ }\NormalTok{Q1W }\OperatorTok{-}\StringTok{ }\NormalTok{EY1 }
\NormalTok{D0 <-}\StringTok{ }\NormalTok{(}\DecValTok{1} \OperatorTok{-}\StringTok{ }\NormalTok{data}\OperatorTok{$}\NormalTok{cmbd) }\OperatorTok{*}\StringTok{ }\NormalTok{(data}\OperatorTok{$}\NormalTok{Y }\OperatorTok{-}\StringTok{ }\NormalTok{Q0W) }\OperatorTok{/}\StringTok{ }\NormalTok{(}\DecValTok{1} \OperatorTok{-}\StringTok{ }\NormalTok{gW) }\OperatorTok{+}\StringTok{ }\NormalTok{Q0W }\OperatorTok{-}\StringTok{ }\NormalTok{EY0}
\CommentTok{# AIPTW CRR 95%CI}
...
\NormalTok{varHat <-}\StringTok{ }\KeywordTok{var}\NormalTok{(EIC) }\OperatorTok{/}\StringTok{ }\NormalTok{n}
\NormalTok{CI <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\KeywordTok{exp}\NormalTok{(logMCRR }\OperatorTok{-}\StringTok{ }\FloatTok{1.96} \OperatorTok{*}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(varHat)), }\KeywordTok{exp}\NormalTok{(logMCRR }\OperatorTok{+}\StringTok{ }\FloatTok{1.96} \OperatorTok{*}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(varHat))); MCRR; CI}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{\{}
\NormalTok{    dat=data[indices,]}
\NormalTok{    gm <-}\StringTok{ }\KeywordTok{glm}\NormalTok{(Y }\OperatorTok{~}\StringTok{ }\NormalTok{age }\OperatorTok{+}\StringTok{ }\NormalTok{cmbd, }\DataTypeTok{data =}\NormalTok{ dat, }\DataTypeTok{family =} \KeywordTok{binomial}\NormalTok{(}\DataTypeTok{link=}\NormalTok{logit))}
    \CommentTok{# Prediction for E(Y|A,W), and E(Y|cmbd = 1, age) and, E(Y|cmbd = 0, age)}
\NormalTok{    Q1W <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(gm, }\DataTypeTok{newdata =} \KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{cmbd =} \DecValTok{1}\NormalTok{, }\DataTypeTok{age =}\NormalTok{ dat[ ,}\KeywordTok{c}\NormalTok{(}\StringTok{"age"}\NormalTok{)]), }\DataTypeTok{type =} \StringTok{"response"}\NormalTok{)}
\NormalTok{    Q0W <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(gm, }\DataTypeTok{newdata =} \KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{cmbd =} \DecValTok{0}\NormalTok{, }\DataTypeTok{age =}\NormalTok{ dat[ ,}\KeywordTok{c}\NormalTok{(}\StringTok{"age"}\NormalTok{)]), }\DataTypeTok{type =} \StringTok{"response"}\NormalTok{)}
    \CommentTok{# Step 2 estimation and prediction of the propensity score (ps): E(A|W) or E(cmbd|age)}
\NormalTok{    psm <-}\StringTok{ }\KeywordTok{glm}\NormalTok{(cmbd }\OperatorTok{~}\StringTok{ }\NormalTok{age, }\DataTypeTok{family =}\NormalTok{ binomial, }\DataTypeTok{data =}\NormalTok{ dat)}
\NormalTok{    gW =}\StringTok{ }\KeywordTok{predict}\NormalTok{(psm, }\DataTypeTok{type =} \StringTok{"response"}\NormalTok{)}
    \CommentTok{# MCRR}
\NormalTok{    EY1 <-}\StringTok{ }\KeywordTok{mean}\NormalTok{((data}\OperatorTok{$}\NormalTok{Y) }\OperatorTok{*}\StringTok{ }\NormalTok{(data}\OperatorTok{$}\NormalTok{Y }\OperatorTok{-}\StringTok{ }\NormalTok{Q1W) }\OperatorTok{/}\StringTok{ }\NormalTok{gW }\OperatorTok{+}\StringTok{ }\NormalTok{Q1W)}
\NormalTok{    EY0 <-}\StringTok{ }\KeywordTok{mean}\NormalTok{((}\DecValTok{1} \OperatorTok{-}\StringTok{ }\NormalTok{data}\OperatorTok{$}\NormalTok{Y) }\OperatorTok{*}\StringTok{ }\NormalTok{(data}\OperatorTok{$}\NormalTok{Y }\OperatorTok{-}\StringTok{ }\NormalTok{Q0W) }\OperatorTok{/}\StringTok{ }\NormalTok{(}\DecValTok{1} \OperatorTok{-}\StringTok{ }\NormalTok{gW) }\OperatorTok{+}\StringTok{ }\NormalTok{Q0W)}
\NormalTok{    MCRR <-}\StringTok{ }\NormalTok{EY1 }\OperatorTok{/}\StringTok{ }\NormalTok{EY0 }
\NormalTok{\}}
\CommentTok{# Can get original estimate, by plugging in indices 1:n}
